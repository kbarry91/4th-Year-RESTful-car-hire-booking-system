...
public class BookingMarshal {
	...
	protected static Bookings getBookingsFromXML(String input) { // Method to unmarshal a list of bookings to xml(For
																	// the GET() method that returns all bookings.)
		...
		try {
			JAXBContext jc = JAXBContext.newInstance(Bookings.class); // Point the JAXBContext to the Bookings class
																		// generated by the schema.
			um1 = jc.createUnmarshaller(); // Initialize the Marshaller.
			StreamSource source1 = new StreamSource(sr1);
			JAXBElement<Bookings> bookingElement1 = um1.unmarshal(source1, Bookings.class); // Unmarshal the xml to the
																							// StringReader.
